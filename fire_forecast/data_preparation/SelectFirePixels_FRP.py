...
if os.path.isfile(outputpath.split('.')[0]+"SampleCoords.nc"):
    sample_coordinates_dataset = xr.open_dataset(outputpath.split('.')[0]+"SampleCoords.nc")
else:
    ...
    sample_latitudes_shifts = np.arange(-latitude_range,latitude_range + grid_size, grid_size) #start, end , step
    sample_latitudes = sample_coordinates[0][:,np.newaxis] + sample_latitudes_shifts[np.newaxis, :]
    sample_longitudes_shifts = np.arange(-longitude_range,longitude_range + grid_size, grid_size) #start, end , step
    sample_longitudes = sample_coordinates[1][:,np.newaxis] + sample_longitudes_shifts[np.newaxis, :]
    ...
    sample_coordinates_dataset = sample_coordinates_dataset.where(
        (
        sample_coordinates_dataset.latitude.min("latitude_pixel")
            >= DS.latitude.min()
        )
        & (
